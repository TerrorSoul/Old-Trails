...
const trailmakersVersions = [
    { name: "2.2 Tank Tracks and Frozen Tracks", manifestId: "3839000052030136885", requiresModifiedSteam: false },
    { name: "2.1 PvP Update: Part 2", manifestId: "337163762234667347", requiresModifiedSteam: false },
    { name: "2.0 Pioneers", manifestId: "4472423625208331844", requiresModifiedSteam: false },
...
const patchUnityCVE = async (versionPath) => {
    const stringsToPatch = [
        { original: '-xrsdk-pre-init-library', replacement: '-xrsdk-pre-init-patched' },
        { original: '-sri-asset-bundle-decryption', replacement: '-sri-asset-bundle-disabled---' }
    ];

    const unityPlayerPaths = findFileRecursive(versionPath, 'UnityPlayer.dll');
    for (const dllPath of unityPlayerPaths) {
        try {
            const data = await fsp.readFile(dllPath);
            let patched = false;

            for (const { original, replacement } of stringsToPatch) {
                const targetBuffer = Buffer.from(original, 'utf8');
                const replaceBuffer = Buffer.from(replacement, 'utf8');
                let offset = data.indexOf(targetBuffer);
                while (offset !== -1) {
                    replaceBuffer.copy(data, offset);
                    patched = true;
                    offset = data.indexOf(targetBuffer, offset + targetBuffer.length);
                }
            }

            if (patched) {
                await fsp.writeFile(dllPath, data);
            }
        } catch (err) {
            console.error(`Failed to patch Unity CVE in ${dllPath}:`, err);
        }
    }
};
...
app.whenReady().then(() => {
    ...
    mainWindow.webContents.once('did-finish-load', async () => {
        ...
        if (isWindows && STEAM_TRAILMAKERS_PATH) {
            if (store.get('isGameActive', false)) {
                mainWindow.webContents.send('status-update', 'Recovering from previous crash...');
                await restoreMainSave();
                await restoreSteamDirectory();
                store.set('isGameActive', false);
            }
...
ipcMain.on('close-window', async () => {
    ...
    if (isSteamDirectoryModified) {
        ...
        if (activeGameVersionName) await saveVersionSession(activeGameVersionName);
        await restoreMainSave();
        await restoreSteamDirectory();
        store.set('isGameActive', false);
    }
...
ipcMain.on('start-download', async (event, { username, password, version }) => {
    ...
    const handleOutput = (data) => {
        ...
        else if (output.includes('Unable to login') || output.includes('InvalidPassword') || output.includes('LogonDenied') || output.includes('AccessDenied')) {
            customError = 'Steam login authorization is expired/invalid. Please re-enter your password.';
    ...
    downloadProcess.on('close', async (code) => {
        ...
        if (code === 0) {
            try {
                ...
                sendStatus('Applying CVE-2025-59489 patch...');
                await patchUnityCVE(finalVersionPath);
            ...
            const finalError = customError || (stderrOutput.trim() || `Process exited with code ${code}.`);
            sendStatus(`Download failed: ${finalError}`);
            event.reply('download-complete', { success: false, message: finalError });
        }
